...
| First-class fn (Phase A) | `T1 -> T2` 型の関数値を closure struct (`closure_int_int` = `{ void* env; int (*fn)(void*, int); }`) で表現。各 top-level fn に `_closure_fn` adapter + `_as_value` const を自動生成、`Var f` を value 位置で使うと `f_as_value` を emit、closure dispatch は `(c).fn((c).env, x)` で。Direct call (`App (Var "f", _)` で `f` が known top-level) は引き続き直接呼出 (最適化) |

### 動かないこと (今のところ Codegen_error で reject)

- inner / anonymous fn を値として返したり別変数に再束ねする (closure-of-captures-as-value)
...
| inner / anonymous fn の値化 (Phase B) | inner fn が値として使われる場合に env struct + closure adapter を生成、ヒープ確保で env を保持 |
