...
//
// Token definitions matching Mere's lexer (lib/lexer.ml). Registers "mere"
// in Prism.js (https://prismjs.com/) `Prism.languages` so the docs site's
// `<pre><code class="language-mere">` blocks get highlighted.
//
// Usage: <script src="prism-core.min.js"></script>
//        <script src="prism-mere.js"></script>
//        Load after Prism core.
...
(function (Prism) {
  Prism.languages.mere = {
    'comment': [
      // single line // ...
      { pattern: /\/\/.*/, greedy: true }
      // Mere has no /* ... */ block comments
    ],
    'string': {
      // String literal "...". Includes `\{` escape and `{expr}` interpolation.
      pattern: /"(?:\\.|[^"\\])*"/,
      greedy: true,
      inside: {
        'interpolation': {
    ...
    'constructor': {
      // Uppercase-leading identifier = constructor (Some / None / Cons / Nil / user-defined)
  ...
  // Prism.js interpolation needs the string's rest to reference the
  // string definition itself recursively.
